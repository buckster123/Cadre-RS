# PRD — **Cadre**: a Rust-native CAD toolkit for AI agents
...
| **Status** | Draft v0.1 |
| **Date** | 2026-07-28 |
| **Working name** | `cadre` (CAD Runtime & Engine) — placeholder pending trademark / crates.io check |
...
Its core design decisions, which Cadre must preserve conceptually:
...
| Pain in the reference architecture | Cadre answer |
|---|---|
| Python environment per skill (interpreter versions, `requirements.txt`, OCP wheels ~hundreds of MB) | One static binary + optional downloadable kernel backend; `cadre` works minutes after `curl` |
...
* **L2 — Drop-in agent compatibility (required for 1.0):** `cadre skills export` produces a skill/plugin pack installable into Claude Code and Codex whose doctrine mirrors the reference structure (progressive references, mandatory snapshot review, viewer handoff), with tool invocations rewritten to `cadre` commands.
* **Explicit non-goal:** source compatibility. build123d Python files do not run on Cadre (see §16 open question on a migration assistant).
...
* G2. Full reference-benchmark parity: all 10 canonical parts buildable and passing deterministic assertions (§12) with an agent driving Cadre.
...
* G4. Deterministic, hermetic model evaluation: same source + same Cadre version ⇒ byte-identical IR and equal-within-tolerance geometry, with no network or filesystem access from model code.
...
* **P3 — The toolsmith.** Embeds `cadre-kernel`/`cadre-lang` in a Rust app, or fronts the HTTP API from a web product.

Representative scenarios (each maps to acceptance tests):

* S1. "Create a centered 100×60×20 mm block with four 8 mm through-holes and a 2 mm top chamfer" → agent writes `block.cad.star`, `cadre build`, checks facts (volume, hole count), snapshots, returns STEP + viewer link.
...
* **P1. Machine-first I/O.** Human-pretty output is a rendering of the JSON, never the other way around. `cadre schema` dumps all command/tool/API schemas.
...
|---|---|
| `cadre-kernel` | `GeomKernel` trait: topology model, boolean/feature ops, queries, tessellation, STEP I/O contract |
| `cadre-occt` | Default kernel backend binding Open CASCADE (OCCT) via FFI; loadable/dynamically linked (§11 licensing) |
| `cadre-truck` | Experimental pure-Rust backend (truck) behind the same trait; feature-gated, non-default |
| `cadre-lang` | Starlark host, CAD stdlib, params, diagnostics, IR emission |
| `cadre-model` | Feature IR, content hashing, selector scheme, build cache, artifact registry |
| `cadre-inspect` | facts / planes / measure / align / frame / diff engines |
| `cadre-render` | Offscreen wgpu tessellated rendering: multi-view PNG packets, orbit GIF/MP4 |
| `cadre-export` | STEP AP242 write (+AP214 compat read), STL, 3MF, GLB, DXF writer |
| `cadre-viewer` | Embedded local web viewer (static assets in binary): serves tessellated GLB + overlays for G-code/DXF/URDF/SRDF/SDF/implicit; file-watch reload; joint jog controls |
| `cadre-parts` | `PartProvider` trait; hosted STEP-catalog client; cache + `parts.lock` with checksums |
| `cadre-robot` | URDF/SRDF/SDF generation stdlib + validators; inertia-from-geometry |
| `cadre-fab` | DFM rulepacks + vendor profiles; slicer discovery/orchestration; G-code static analysis; printer adapters (Bambu LAN FTPS/MQTT first) |
| `cadre-mcp` | MCP server (official Rust SDK), stdio + streamable HTTP |
| `cadre-api` | Axum HTTP API, jobs, SSE, OpenAPI generation |
| `cadre-cli` | Clap front end; the `cadre` binary |
| `cadre-skills` | Skill-pack generator/exporter for Claude Code & Codex; bundled doctrine docs |
...
```python
# block.cad.star  —  built with: cadre build block.cad.star
...
* **FR-101 (P0).** `cadre build <target.star> [-o out]` evaluates the source hermetically, executes the IR on the kernel, and writes the primary artifact next to the source with the same basename (`block.cad.star` → `block.step`). Directory-wide builds are refused.
...
* **FR-105 (P0).** Build cache keyed by (source hash, params, cadre+kernel versions); warm no-op rebuilds return in < 100 ms.
...
* **FR-107 (P1).** `cadre watch <target>` rebuilds on change and pushes to the viewer.
...
* **FR-301 (P0).** `cadre snapshot <target> [--views iso,front,top,right] [--focus <ref>] [--out dir]`: headless PNG packet with consistent framing, edges overlaid, and a scale/annotation footer; deterministic camera derivation from bbox.
...
* **FR-404 (P1).** DXF writer (R2018 text) for `gen_dxf()` sources and for projections of planar faces/silhouettes from 3D targets (`cadre export --dxf --face <ref>`); layer/units conventions documented.
* **FR-405 (P2).** OBJ/PLY export; glTF instancing for patterned parts.

### 7.5 Viewer (FR-5xx)

* **FR-501 (P0).** `cadre view [path…] [--port]` serves an embedded local web viewer; returns per-file deep links; reuses a running instance rather than spawning duplicates.
...
* **FR-802 (P0).** First vendor profile: SendCutSend-style laser/CNC/bending checks (material+thickness availability from a profile file, min hole size vs thickness, min bridge/web width, bend-relief and flange-length rules, tapping/countersink/hardware constraints, flat-pattern closure). Profiles are data (TOML/JSON), community-updatable, and versioned — Cadre never claims live vendor truth, only profile-version truth.
* **FR-803 (P0).** Slicer orchestration: discover local PrusaSlicer/OrcaSlicer/Bambu Studio CLIs; `cadre fab slice <mesh> --slicer … --printer-profile … --filament …` runs the real slicer, captures logs, and returns the `.gcode` + summary (est. time, filament, bbox). Cadre does not reimplement slicing.
...
| Command | Purpose |
|---|---|
| `cadre init` | Scaffold project layout + config |
| `cadre build <target.star \| .step> [--set k=v] [-o]` | Build primary artifact (FR-101…) |
| `cadre watch <target>` | Rebuild-on-change + viewer push |
| `cadre inspect refs\|measure\|align\|frame\|diff …` | Numeric interrogation (FR-2xx) |
| `cadre snapshot <target> [--views…] [--gif]` | Visual review packets (FR-3xx) |
| `cadre export <target> --format step\|stl\|3mf\|glb\|dxf [--face <ref>]` | Secondary exports (FR-4xx) |
| `cadre view [paths…]` | Start/reuse local viewer, print links |
| `cadre parts search\|show\|fetch\|lock …` | Catalog sourcing (FR-6xx) |
| `cadre robot validate <file.urdf\|.srdf\|.sdf> [--against <pair>]` | Robotics validators (FR-7xx) |
| `cadre fab check <dxf\|step> --profile <vendor@ver>` | DFM preflight (FR-801/802) |
| `cadre fab slicers` / `cadre fab slice …` / `cadre fab gcode-check <file>` | Slicing path (FR-803/804) |
| `cadre printer status\|upload\|dry-run\|start\|watch …` | Printer handoff (FR-805/806, gated) |
| `cadre serve api [--port] [--token …]` | HTTP API server |
| `cadre mcp [--http --port]` | MCP server (stdio default) |
| `cadre skills export --agent claude-code\|codex [--out]` / `cadre skills install` | L2 skill packs |
| `cadre bench run [--suite parity10]` / `cadre bench agent --cmd …` | Verification suites (§12) |
| `cadre engine install\|info` | Fetch/inspect kernel backend (§11 licensing/distribution) |
| `cadre schema [command\|mcp\|api]` | Machine-readable self-description |
...
Tool catalog (names final, schemas in `cadre schema mcp`; Appendix B sketches three):
...
Resources: `cadre://project/**` sources, `cadre://artifact/**` outputs, `cadre://doc/**` doctrine pages (so MCP-only agents can progressively load the same references the skill pack ships).
...
```
myproject/
  cadre.toml            # config: kernel, viewer port, printers allow-list, providers
  ...
  .cadre/               # build cache, tessellation cache, logs (gitignored)
```

Paths in commands resolve from the invoking CWD, never from install/skill directories. Config precedence: flags > env (`CADRE_*`) > project `cadre.toml` > user config. No telemetry keys exist.

---

## 10. Error model

Every failure is a diagnostic object: `{ code: "CADRE-E-FILLET-RADIUS", severity, message, target, span: {file,line,col}, refs: ["#o1.1.e12"], hint, docs_url }`. Requirements: codes are stable and enumerable (`cadre schema errors`); kernel failures are translated into feature-level terms (which op, which selector, plausible causes) rather than raw kernel traces; hints are actionable ("reduce radius below 2.4 mm, the minimum adjacent edge length"). Diagnostic quality is a benchmarked feature: the agent-repair benchmark (§12) measures loop count to fix seeded errors.
...
* **NFR-2 Footprint.** Core binary ≤ 40 MB; OCCT backend as a separately fetched component (`cadre engine install`, checksummed) ≤ 150 MB installed.
...
* **NFR-9 No telemetry.** None. A `cadre bench` opt-in flag may print a shareable local report; nothing phones home.
...
4. **Agent-in-the-loop harness** (`cadre bench agent`): drives a configurable agent (any MCP client or CLI-scripted model) against the Parity-10 *prompts* (natural language only), scoring first-try success, loops-to-success, wall time, and token cost. This is the number that answers "is it actually good for AI?"
...
|---|---|---|
| OCCT FFI maintenance burden / build complexity per platform | Schedule, stability | Isolate at `GeomKernel`; prebuilt engine artifacts via `cadre engine install`; M0 go/no-go includes a vendored-binding fallback plan |
...
| LLMs less fluent authoring the Cadre stdlib than build123d | Agent scores lag | Python-shaped Starlark; rich few-shot examples in skill pack; error hints name the correct API; agent harness tracked from M2, stdlib adjusted empirically |
...
3. Migration assistant: a best-effort `cadre migrate <build123d.py>` that emits a *skeleton* `.cad.star` (structure + parameters, human/agent completes it) — valuable, but must be built without reading reference source (train on build123d's own public docs only). Include in M6 or defer?
...
| Reference skill | Essence | Cadre equivalent | Phase |
|---|---|---|---|
| CAD | build123d `gen_step()` sources; STEP-first; `step`/`inspect`/`snapshot` tools; selectors; assemblies+joints; defaults doctrine | `cadre-lang` `.cad.star` + `build`/`inspect`/`snapshot`; selector scheme; assemblies (FR-106); doctrine in skill pack | M1–M3 |
| CAD Viewer | Local browser review for CAD/G-code/robot files; live links; reuse running instance | `cadre view` embedded viewer (FR-5xx) | M2–M4 |
| step.parts | Hosted catalog search/fetch of purchasable STEP parts, checksums, envelope-on-miss doctrine | `cadre parts` + `PartProvider` + `parts.lock` (FR-6xx) | M3 |
...
| URDF | `gen_urdf()` + generation-time validation; frames/inertials as first-class risks | `cadre-robot` URDF gen/validate + inertia-from-geometry (FR-701–703) | M4 |
...
| Skills/plugin distribution | `SKILL.md` packs installed into Claude Code/Codex marketplaces | `cadre skills export/install` (L2, §8.4) | M2–M6 |
...
```
$ cadre build cad/flange.cad.star --json
{"ok":false,"diagnostics":[{"code":"CADRE-E-FILLET-RADIUS","span":{"file":"cad/flange.cad.star","line":14},
...
$ cadre build cad/flange.cad.star --json          # ok:true, facts attached
$ cadre inspect measure cad/flange.step '#o1.2' --kind diameter --json   # 30.0
$ cadre snapshot cad/flange.step --views iso,front --gif
$ cadre view cad/flange.step                       # → http://127.0.0.1:7411/f/flange.step
```
