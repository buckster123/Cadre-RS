...
impl Shape {
    ...
    /// Deep-copy geometry via `BRepBuilderAPI_Transform` (identity, copy=true).
    /// Avoids STEP round-trips for clone.
    pub fn deep_copy(&self) -> Result<Self, Error> {
        let transform = ffi::new_transform();
        self.apply_transform(&transform)
    }

    /// Apply a prepared `gp_Trsf` and return a new shape (always copies geometry).
    pub fn apply_transform(&self, transform: &ffi::gp_Trsf) -> Result<Self, Error> {
        let mut brep = ffi::BRepBuilderAPI_Transform_ctor(&self.inner, transform, true);
        let progress = ffi::Message_ProgressRange_ctor();
        brep.pin_mut().Build(&progress);
        if !brep.IsDone() {
            return Err(Error::TransformFailed);
        }
        let shape = brep.pin_mut().Shape();
        Ok(Self {
            inner: ffi::TopoDS_Shape_to_owned(shape),
        })
    }

    /// Build and apply a transform in one call.
    pub fn transformed_with(
        &self,
        setup: impl FnOnce(std::pin::Pin<&mut ffi::gp_Trsf>),
    ) -> Result<Self, Error> {
        let mut transform = ffi::new_transform();
        setup(transform.pin_mut());
        self.apply_transform(&transform)
    }

    /// Unit sphere at origin with given radius.
    pub fn sphere(radius: f64) -> Result<Self, Error> {
        let mut make = ffi::BRepPrimAPI_MakeSphere_ctor(radius);
        let progress = ffi::Message_ProgressRange_ctor();
        make.pin_mut().Build(&progress);
        if !make.IsDone() {
            return Err(Error::PrimitiveFailed);
        }
        let shape = make.pin_mut().Shape();
        Ok(Self {
            inner: ffi::TopoDS_Shape_to_owned(shape),
        })
    }
