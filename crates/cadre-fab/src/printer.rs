//! Printer adapters — Bambu LAN with hard start gates; live I/O opt-in.
...
impl Default for ExternalLiveTransport {
    fn default() -> Self {
        Self {
            curl: PathBuf::from(std::env::var("CADRE_CURL").unwrap_or_else(|_| "curl".into())),
            mosquitto_pub: PathBuf::from(
                std::env::var("CADRE_MOSQUITTO_PUB").unwrap_or_else(|_| "mosquitto_pub".into()),
            ),
            timeout_secs: 60,
        }
    }
    ...
    pub fn detect() -> Result<Self, PrinterError> {
        ...
        which_ok(&t.curl).map_err(|e| {
            PrinterError::Transport(format!(
                "curl not found ({e}); install curl or set CADRE_CURL"
            ))
        })?;
        which_ok(&t.mosquitto_pub).map_err(|e| {
            PrinterError::Transport(format!(
                "mosquitto_pub not found ({e}); install mosquitto-clients or set CADRE_MOSQUITTO_PUB"
            ))
        })?;
        Ok(t)
    }
...
fn tempfile_path(remote_name: &str) -> Result<PathBuf, PrinterError> {
    ...
    Ok(dir.join(format!("cadre-bambu-{safe}")))
}
...
impl BambuAdapter {
    ...
    /// Build from CLI/env: access code from arg or `CADRE_BAMBU_ACCESS_CODE`.
    pub fn from_env(
        id: impl Into<String>,
        ...
        let code = access_code.or_else(|| std::env::var("CADRE_BAMBU_ACCESS_CODE").ok());
        let serial = serial.or_else(|| std::env::var("CADRE_BAMBU_SERIAL").ok());
...
impl Printer for BambuAdapter {
    ...
    fn dry_run(
        ...
        if self.access_code.is_none() {
            notes.push("no access code set (CADRE_BAMBU_ACCESS_CODE or --access-code)".into());
        }
        if self.info.serial.is_none() {
            notes.push("no serial set (CADRE_BAMBU_SERIAL or --serial) — required for MQTT".into());
        }
        if !gcode_check.ok {
        ...
        let access = match &self.access_code {
            ...
            _ => {
                return Ok(StartGate {
                    ok: false,
                    errors: vec![
                        "live start needs access code (--access-code or CADRE_BAMBU_ACCESS_CODE)"
                            .into(),
                    ],
                    live_attempted: false,
                    uploaded_as: None,
        ...
        let serial = match &self.info.serial {
            ...
            _ => {
                return Ok(StartGate {
                    ok: false,
                    errors: vec![
                        "live start needs printer serial (--serial or CADRE_BAMBU_SERIAL)".into(),
                    ],
                    live_attempted: false,
                    uploaded_as: None,
                    mqtt_topic: None,
