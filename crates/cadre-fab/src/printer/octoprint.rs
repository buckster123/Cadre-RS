...
impl OctoPrintAdapter {
    ...
    pub fn from_env(
        ...
        let key = api_key.or_else(|| std::env::var("CADRE_OCTOPRINT_API_KEY").ok());
        let mut a = Self::new(id, host, model);
        if let Some(u) = base_url.or_else(|| std::env::var("CADRE_OCTOPRINT_URL").ok()) {
            a = a.with_base_url(u);
        }
        if let Some(k) = key {
            a = a.with_api_key(k);
...
impl Printer for OctoPrintAdapter {
    ...
    fn status(&self) -> Result<serde_json::Value, PrinterError> {
        Ok(json!({
            ...
            "gates": {
                ...
                "api_key": "required for live (CADRE_OCTOPRINT_API_KEY)"
            }
        }))
    }
    ...
    fn dry_run(
        ...
        if self.api_key.is_none() {
            notes.push(
                "no API key set (CADRE_OCTOPRINT_API_KEY or --api-key) — required for live".into(),
            );
        }
        if !gcode_check.ok {
        ...
        let key = match &self.api_key {
            ...
            _ => {
                return Ok(StartGate {
                    ok: false,
                    errors: vec![
                        "live start needs API key (--api-key or CADRE_OCTOPRINT_API_KEY)".into(),
                    ],
                    live_attempted: false,
                    uploaded_as: None,
                    mqtt_topic: None,
