...
impl KlipperAdapter {
    ...
    pub fn from_env(
        ...
        let key = api_key.or_else(|| std::env::var("CADRE_MOONRAKER_API_KEY").ok());
        let mut a = Self::new(id, host, model);
        if let Some(u) = base_url.or_else(|| std::env::var("CADRE_MOONRAKER_URL").ok()) {
            a = a.with_base_url(u);
        }
        if let Some(k) = key {
            a = a.with_api_key(k);
...
impl Printer for KlipperAdapter {
    ...
    fn dry_run(
        ...
        if self.api_key.is_none() {
            notes.push(
                "no API key set (CADRE_MOONRAKER_API_KEY or --api-key) — optional if Moonraker open"
                    .into(),
            );
        }
        if !gcode_check.ok {
