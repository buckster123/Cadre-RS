...
use cadre_inspect::{inspect_refs, TopologySnapshot};
use cadre_kernel::{GeomKernel, MockKernel, ShapeFacts};
...
pub fn run_task(task: &Task, tasks_root: &Path) -> Result<TaskResult, String> {
    let started = Instant::now();
    let work =
        std::env::temp_dir().join(format!("cadre-harness-{}-{}", task.id, std::process::id()));
