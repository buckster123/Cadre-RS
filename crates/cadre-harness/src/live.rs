...
//! - `CADRE_HARNESS_TASK_ID`, `CADRE_HARNESS_PROMPT`
//! - `CADRE_HARNESS_WORKDIR`, `CADRE_HARNESS_PART` (path to `part.cad.star`)
//! - `CADRE_HARNESS_LOOP`, `CADRE_HARNESS_MAX_LOOPS`
//! - `CADRE_HARNESS_TASK_FILE` (path to task JSON, read-only)
//!
//! The agent must leave a valid `.cad.star` at `CADRE_HARNESS_PART`. The harness then
...
fn run_agent_cmd(
    ...
    cmd.env("CADRE_HARNESS_TASK_ID", &task.id)
        ...
        .env("CADRE_HARNESS_LOOP", loop_n.to_string())
        .env("CADRE_HARNESS_MAX_LOOPS", max.to_string())
        .env("CADRE_HARNESS_TASK_FILE", task_file)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
