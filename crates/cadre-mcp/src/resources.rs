//! MCP resources: `cadre://doc/**` and `cadre://artifact/**`.
...
pub fn list_resources(project_root: &Path) -> Value {
    let mut resources = vec![
        res("cadre://doc/status", "Cadre live status", "text/markdown"),
        res("cadre://doc/stdlib", "Stdlib depth (H2)", "text/markdown"),
        res(
            "cadre://doc/viewer",
        ...
        res(
            "cadre://doc/write-source-policy",
            "write_source / read_source policy (H7)",
            "text/markdown",
        ),
        res(
            "cadre://artifact/index",
...
pub fn read_resource(project_root: &Path, uri: &str) -> Result<Value, ToolError> {
    if let Some(rest) = uri.strip_prefix("cadre://doc/") {
    ...
    if let Some(rel) = uri.strip_prefix("cadre://artifact/file/") {
...
fn scan_artifacts(root: &Path) -> Vec<(String, String)> {
    ...
    fn walk(dir: &Path, root: &Path, out: &mut Vec<(String, String)>, depth: u32) {
        ...
        for e in rd.flatten() {
            ...
            if name.starts_with('.') && name != ".cadre" {
                continue;
            }
            if p.is_dir() {
            ...
            {
                out.push((format!("cadre://artifact/file/{rel_s}"), rel_s.clone()));
            }
        }
    }
...
fn doc_body(id: &str, project_root: &Path) -> Result<String, ToolError> {
    ...
    Ok(format!(
        "# {id}\n\n_(doc file `{rel}` not found on disk; open the Cadre-RS repo.)_\n"
    ))
}
...
| **HTTP** (`cadre serve mcp`) | **ON** | `CADRE_MCP_WRITE_SOURCE=0` to disable |
...
- `cadre://doc/**` — bundled doctrine markdown
- `cadre://artifact/index` — scan of `.cad.star` / `.ir.json` / snaps / gcode under project root
- `cadre://artifact/file/<relpath>` — read one artifact (path-escape refused)
"#;
