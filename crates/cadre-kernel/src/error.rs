...
//!
//! Codes are stable strings agents branch on (`CADRE-E-…`). Keep them aligned with
//! `cadre schema errors` once that surface exists.
...
pub enum KernelError {
    /// Structured diagnostic with stable `CADRE-E-*` code.
    #[error("{code}: {message}")]
    Diagnostic {
        /// Stable code, e.g. `CADRE-E-FILLET-RADIUS`.
        code: String,
...
impl KernelError {
    /// Stable machine code when present (`CADRE-E-…`), else a coarse class name.
    pub fn code(&self) -> &str {
        match self {
            Self::Diagnostic { code, .. } => code.as_str(),
            Self::UnknownShape { .. } => "CADRE-E-UNKNOWN-SHAPE",
            Self::UnknownEdge { .. } => "CADRE-E-UNKNOWN-EDGE",
            Self::Unsupported { .. } => "CADRE-E-UNSUPPORTED",
            Self::InvalidArg { .. } => "CADRE-E-INVALID-ARG",
            Self::Io { .. } => "CADRE-E-IO",
        }
    }
