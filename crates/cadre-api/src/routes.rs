...
async fn parts_search(
    ...
    auth(&st, &headers)?;
    use cadre_parts::{LocalFsProvider, PartProvider};
    let root = body
        .parts_root
        .map(PathBuf::from)
...
async fn assembly_validate(
    ...
    let assy: cadre_parts::AssemblySpec =
    ...
    if lock_path.is_file() {
        let lock =
            cadre_parts::load_parts_lock(&lock_path).map_err(|e| ApiError::bad(e.to_string()))?;
        for c in &assy.components {
            if c.from_lock {
                match cadre_parts::verify_lock_entry(&lock, &c.source, &st.cfg.project_root) {
        ...
        "build" | "inspect_refs" | "snapshot" => {
            ...
            match path {
                Some(path) => {
                    ...
                    match cadre_mcp::tools_call_for_api(&kind, &args) {
                        Ok(v) => Ok(v),
                        Err(e) => Err(e),
                    }
                }
...
fn call_tool(name: &str, args: &Value) -> Result<Json<Value>, ApiError> {
    match cadre_mcp::tools_call_for_api(name, args) {
        Ok(v) => Ok(Json(v)),
