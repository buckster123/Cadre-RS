...
cargo run -p cadre-cli -- build path/to/part.cad.star --json
cargo run -p cadre-cli -- inspect refs path/to/part.cad.star --facts --json
...
cargo run -p cadre-cli --features occt -- --kernel occt export part.cad.star --format stl -o part.stl --json
```
...
- Directory targets refused (`CADRE-E-EXPLICIT-TARGET`).
