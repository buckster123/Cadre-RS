...
use cadre_kernel::StepWriteOpts;
use cadre_lang::{evaluate, execute_ir, EvalOptions, FeatureIr};
use cadre_model::{BuildCache, CacheKey};
...
pub fn run(cli: &Cli, args: &BuildArgs) -> ExitCode {
    ...
    let source = match fs::read_to_string(&target) {
        Ok(s) => s,
        Err(e) => {
            let v = json!({"ok": false, "diagnostics": [{"code": "CADRE-E-IO", "message": e.to_string(), "target": target}]});
            emit(cli.json, &v, false);
            return ExitCode::Io;
        }
    };
    ...
    let overrides = match parse_sets(&args.set) {
        ...
        Err(msg) => {
            let v =
                json!({"ok": false, "diagnostics": [{"code": "CADRE-E-USAGE", "message": msg}]});
            emit(cli.json, &v, false);
            return ExitCode::Usage;
        }
    };
    ...
    let cache_root = project.join(".cadre").join("cache");
            ...
            if let Ok(Some(hit)) = cache.get(&cache_key) {
                ...
                let body = json!({
                    ...
                    "meta": {
                        ...
                        "cadre_version": env!("CARGO_PKG_VERSION"),
                    }
                });
    ...
    artifacts.push(json!({
        "path": ir_out,
        "kind": "ir",
        "sha256": cadre_model::sha256_hex(&ir_json),
    }));
    ...
    if step_ok {
        match fs::read(&step_out) {
            Ok(b) => {
                let hash = cadre_model::sha256_bytes(&b);
                artifacts.push(json!({
                    "path": step_out,
                    "kind": "step",
                    "sha256": hash,
            ...
            Err(e) => {
                diagnostics.push(json!({"code": "CADRE-E-IO", "message": e.to_string()}));
            }
        }
    }
