...
pub fn run(cli: &Cli, args: &SnapshotArgs) -> ExitCode {
    ...
    if !target.exists() {
        let v = json!({"ok": false, "diagnostics": [{"code": "CADRE-E-IO", "message": format!("not found: {}", target.display())}]});
    ...
    let source = match fs::read_to_string(target) {
        Ok(s) => s,
        Err(e) => {
            let v = json!({"ok": false, "diagnostics": [{"code": "CADRE-E-IO", "message": e.to_string()}]});
            emit(cli.json, &v, false);
            return ExitCode::Io;
        }
    };
    ...
    let (mesh, notes) = match mesh_from_ir(&ir) {
        Ok(m) => m,
        Err(e) => {
            let v = json!({"ok": false, "diagnostics": [{"code": "CADRE-E-RENDER", "message": e}]});
            emit(cli.json, &v, false);
    ...
    if views.is_empty() {
        let v = json!({"ok": false, "diagnostics": [{"code": "CADRE-E-USAGE", "message": "no valid --views"}]});
        emit(cli.json, &v, false);
        return ExitCode::Usage;
    }
