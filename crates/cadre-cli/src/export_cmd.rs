...
use cadre_kernel::{GeomKernel, Mesh, StepWriteOpts, TessTol};
    ...
    match args.format {
        ...
        ExportFormat::Stl => match kernel.as_mut().tessellate(shape, TessTol::default()) {
            Ok(mesh) => match write_stl_ascii(&out, &mesh) {
                ...
                Err(e) => {
                    let body = json!({"ok": false, "diagnostics": [{"code": "CADRE-E-IO", "message": e.to_string()}]});
                    emit(cli.json, &body, false);
                    ExitCode::Io
                }
            },
...
) -> Result<cadre_kernel::ShapeId, (ExitCode, serde_json::Value)> {
    let ext = args
        .target
        .extension()
        .and_then(|e| e.to_str())
...
fn write_gltf_json(path: &std::path::Path, mesh: &Mesh) -> std::io::Result<PathBuf> {
    ...
    let gltf = json!({
        "asset": {"version": "2.0", "generator": "cadre-cli"},
        "buffers": [
