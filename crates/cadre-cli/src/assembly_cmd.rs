...
use cadre_parts::{assembly_kinematics, assembly_to_robot_json, validate_assembly, AssemblySpec};
use serde_json::json;
...
fn emit_robot(cli: &Cli, a: &AssemblyEmitArgs) -> ExitCode {
    ...
    emit(
        ...
        &json!({
            ...
            "out": out,
            "robot": robot,
            "note": "placeholder geometry — feed to `cadre robot gen` for URDF/SRDF/SDF",
        }),
        true,
    );
    ExitCode::Ok
