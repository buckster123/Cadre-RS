...
/// Evaluate Cadre Starlark source and produce feature IR.
///
...
fn evaluate_inner(
        ...
        Module::with_temp_heap(|module| -> Result<NodeId, Vec<Diagnostic>> {
            ...
            let gen_step = {
                ...
                module.get("gen_step").ok_or_else(|| {
                    vec![
                        Diagnostic::error("CADRE-E-NO-ENTRY", "module must define gen_step()")
                            .with_target(name)
                            .with_hint(
    ...
    store
        .builder
        .finish(root)
        .map_err(|msg| vec![Diagnostic::error("CADRE-E-IR", msg).with_target(name)])
}
...
"#;
    ...
    fn missing_gen_step_is_structured() {
        ...
        assert!(!r.ok);
        assert_eq!(r.diagnostics[0].code, "CADRE-E-NO-ENTRY");
        let json = serde_json::to_string(&r).unwrap();
        assert!(json.contains("CADRE-E-NO-ENTRY"));
    }
    ...
    fn syntax_error_diagnostic() {
        ...
        assert!(!r.ok);
        assert_eq!(r.diagnostics[0].code, "CADRE-E-EVAL");
    }
...
"#;
        ...
        assert!(!r.ok);
        assert_eq!(r.diagnostics[0].code, "CADRE-E-HERMETIC-LOAD");
    }
